/*
 特务集卡
 脚本没有自动开卡，会尝试领取开卡奖励
cron "31 10,17,19 * * *" jd_superBrandJKK.js
*/
...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        for (let ii = 0; ii < shareList.length; ii++) {
            if (shareList[ii].user === userName) {
                shareArr.push(shareList[ii]);
                break;
            }
        }
    }
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        let userName = decodeURIComponent(ck.match(/pt_pin=(.+?);/) && ck.match(/pt_pin=(.+?);/)[1]);
        let _0x4ce29d = true;
        for (let ii = 0; (ii < shareArr.length) && _0x4ce29d; ii++) {
            let _0x383730 = shareArr[ii];
            if ((_0x383730.user === userName) || (_0x383730.need === 0) || _0x383730.max) {
                continue;
            }
            console.log('' + userName + '去助力:' + _0x383730.user);
            let _0x224f8e = await takeRequest(ck, 'superBrandDoTask', '{"source":"card","activityId":' + _0x383730.activityId + ',"encryptProjectId":"' + _0x383730.encryptProjectId + '","encryptAssignmentId":"' + _0x383730.encryptAssignmentId + '","assignmentType":2,"itemId":"' + _0x383730.itemId + '","actionType":0}');
