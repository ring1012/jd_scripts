...
!(async () => {
  ...
  await client.ListFunctions({}).then(
    async data => {
      ...
      console.log(`创建函数`);
      let inputYML = ".ggghhh/workflows/deploy_tencent_scf.yml";
  ...
  .finally(async () => {
    ...
    if (process.env.GGGHHH_ACTIONS == "true") {
      fs.writeFile(
        "action.js",
        `var action = ` + process.env.action + `;action > 0 ? require("./sendNotify").sendNotify("云函数部署异常！请重试","点击通知，登入后查看详情",{ url: process.env.GGGHHH_SERVER_URL + "/" + process.env.GGGHHH_REPOSITORY + "/actions/runs/" + process.env.GGGHHH_RUN_ID + "?check_suite_focus=true" }): ""`,
        "utf8",
        function (error) {
          if (error) {
            console.log(error);
